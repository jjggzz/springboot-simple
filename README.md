...
# 1.background
  我太难了!!之前在公司上班,受大佬启发@likowong,再加上自己也厌烦了项目中的一大堆配置.虽然springboot已经够简单了,但是也免不了要写点自定义的配置类,就想着能不能把这些重复的东西抽出来,因为配置都大同小异.比如说动态数据源\\全局异常处理\\mysql主从动态数据源\\异步事件调用抽取等等,所以才有了这个东东.
# 2.structure
  ````
  springboot-simple
